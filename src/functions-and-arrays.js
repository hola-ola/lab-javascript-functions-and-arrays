...
function averageNumbers(numbersAvg) {
  ...
  let averageNum = 0;
  let sum = 0;
  for (num of numbersAvg) {
    sum += num;
  } averageNum = sum / numbersAvg.length;
    return averageNum;
};
...
function averageWordLength(wordsArr) {
  ...
  } averageLength = totalLength / wordsArr.length;
    return averageLength;
};

// Bonus: Calculate the average of a mixed elements array

function avg(newArray) {
  let newTotal = 0;
  let newAvg = 0;
  ...
  for (element of newArray) {
    ...
    if (typeof element === "string") {
      newTotal += element.length;
    }
    if (element === true) {
      newTotal += 1;
    }
  } newAvg = newTotal / newArray.length;
...
function howManyTimes(wordsUnique, word) {
  counter = 0;
  if (wordsUnique.length === 0) {
    return 0;
  } for (element of wordsUnique) {
    if (element === word) {
      counter ++
    }
  } return counter
...
function greatestProduct(matrix) {
  ...
  } if (counterDifThan1 == 0) {
      return 1
  } if (counterDifThan2 == 0) {
      return 16
  } else {
      return "ZONK!";
  }
